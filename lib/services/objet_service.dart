...
// Méthode pour ajouter un objet avec des fichiers locaux (mobile)
Future<void> addObjetWithImageFiles(Objet objet, List<File> imageFiles) async {
  try {
    ...
    objet.utilisateur = currentUser;
    objet.dateAjout = DateTime.now();

    List<String> imageUrls = [];

    for (var imageFile in imageFiles) {
      final imageRef = _storage.ref().child('images/${objet.id}/${imageFile.path.split('/').last}');
      await imageRef.putFile(imageFile); // Utilise `putFile` pour les fichiers locaux
      final imageUrl = await imageRef.getDownloadURL();
      imageUrls.add(imageUrl);
    }

    objet.imageUrl = imageUrls.join(',');
...
// Méthode pour mettre à jour un objet avec des fichiers locaux (mobile)
Future<void> updateObjetWithImageFiles(Objet objet, List<File> imageFiles) async {
  try {
    List<String> imageUrls = [];
